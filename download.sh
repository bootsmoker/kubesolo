...
convert_url() {
    ...
    if [[ "$__KSPEEDER_AVAILABLE__" == "true" ]]; then
        if [[ "$url" == https://raw.githubusercontent.com/* ]]; then
            convert_github_raw_url "$url"
            return
        elif [[ "$url" == https://github.com/* ]]; then
            convert_github_url "$url"
            return
        fi
    fi
...
KUBESOLO_INSTALL_URL=$(convert_url "$KUBESOLO_INSTALL_URL_NEW")
if [[ "$__KSPEEDER_AVAILABLE__" == "true" ]]; then
...
wget "$(convert_url "$KUBESOLO_UNINSTALL_URL_NEW")" -O uninstall.sh && chmod +x uninstall.sh \
...
wget "$(convert_url "https://raw.githubusercontent.com/bootsmoker/kubesolo/HEAD/offline/setup.sh")" -O setup.sh && chmod +x setup.sh \
...
wget "$(convert_url "https://raw.githubusercontent.com/bootsmoker/kubesolo/HEAD/install-kubectl.sh")" -O install-kubectl.sh && chmod +x install-kubectl.sh \
...
wget "$(convert_url "https://raw.githubusercontent.com/bootsmoker/kubesolo/HEAD/uninstall-kubectl.sh")" -O uninstall-kubectl.sh && chmod +x uninstall-kubectl.sh \
